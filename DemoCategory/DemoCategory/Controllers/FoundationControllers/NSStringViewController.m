...
- (void)setUI
{
    ...
    NSArray *array = @[@"IP", @"number16转string", @"首字符", @"用*号隐藏指定位", @"保留任意位小数", @"金额转换显示", @"有效字符", @"非空字符", @"过滤空格字符", @"汉字字符", @"字符长度", @"手机号码", @"移动手机号码", @"联通手机号码", @"电信手机号码", @"邮箱", @"空格字符串", @"包含某个子字符串", @"包含中文", @"纯数字字符串", @"数字大小写字母_@的字符串", @"指定金额（2位小数）", @"有效银行卡", @"有效身份证"];
